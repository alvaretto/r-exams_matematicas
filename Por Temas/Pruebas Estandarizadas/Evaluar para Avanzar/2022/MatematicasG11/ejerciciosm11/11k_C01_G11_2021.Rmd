...
porcentaje5 <- porcentaje[3]+porcentaje[4]
porcentaje6 <- porcentaje[1]+porcentaje[2]
exce2 <- sample(c(clientes*porc4-2,clientes*porc4+2),1)
exce <- clientes*porc4
...
bue_exce <- exce+bueno
bue_exce2 <- exce+bueno
regu_malo <- regular+malo
regu_malo2 <- sample(c(regular+malo-2, regular+malo-5),1)
...
La afirmación incorrecta acerca de los resultados de la encuesta es:
...
- Menos de `r regu_malo2` clientes están insatisfechos con el servicio que ofrece la empresa.
<br/> 
...
========
\
Dado que la encuesta se realizó a `r clientes` clientes, y el `r porcentaje6` % de los encuestados calificó como “insatisfactorio” el servicio, entendiendo que los clientes insatisfechos son aquellos que consideran que el servicio es "malo" o "regular", entonces  `r porcentaje[1]`% + `r porcentaje[2]`% = `r porcentaje6`%. 

Además, $\frac{`r porcentaje6`}{100}$ × `r clientes` clientes = `r regu_malo` clientes calificaron como “insatisfactorio” el servicio. Por tanto, la afirmación incorrecta es: "Menos de `r regu_malo2` clientes están insatisfechos con el servicio que ofrece la empresa".
