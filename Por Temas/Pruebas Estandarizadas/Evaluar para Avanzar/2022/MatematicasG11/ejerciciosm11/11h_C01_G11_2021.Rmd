...
i <- 0
while(i==0){
porc <- seq(15, 100, 5) # Números de 5 en 5 hasta 100
...
========
\
Dado que la encuesta se realizó a `r clientes` clientes, y el `r porcentaje5` % de los encuestados calificó como “satisfactorio” el servicio, entendiendo que los clientes satisfechos son aquellos que consideran que el servicio es "bueno" o "excelente", entonces  `r porcentaje[3]`% + `r porcentaje[4]`% = `r porcentaje5`%. 

Además, $\frac{`r porcentaje5`}{100}$ × `r clientes` clientes = `r bue_exce` clientes calificaron como “satisfactorio” el servicio. Por tanto, la afirmación incorrecta acerca de los resultados de la encuesta es: "Menos de `r bue_exce2` clientes están satisfechos con el servicio que ofrece la empresa".
