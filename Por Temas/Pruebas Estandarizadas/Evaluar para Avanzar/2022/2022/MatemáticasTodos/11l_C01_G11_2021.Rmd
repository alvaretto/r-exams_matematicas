...
malo <- clientes*porc1
malo2 <- sample(c(clientes*porc1+2, clientes*porc1+5),1)
bue_exce <- exce+bueno
...
- Más de `r malo2` clientes consideran que la calidad del servicio que ofrece la empresa es mala.
<br/> 

- Menos de `r regular2` clientes consideran que la calidad del servicio que ofrece la empresa es regular.
<br/> 

- Más de `r bue_exce2` clientes están satisfechos con el servicio que ofrece la empresa.
<br/> 
...
========
\
Dado que la encuesta se realizó a `r clientes` clientes, y el `r porcentaje[1]` % de los encuestados calificó como “malo” el servicio, entonces $\frac{`r porcentaje[1]`}{100}$ × `r clientes` clientes = `r malo` clientes calificaron como “malo” el servicio. Por tanto, la afirmación incorrecta es: "Más de `r malo2` clientes consideran que la calidad del servicio que ofrece la empresa es mala".
\
