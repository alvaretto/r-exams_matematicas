...
\\pgfplotsset{width=7cm,compat=1.17}
	\\begin{tikzpicture}
		\\begin{axis}[
			...
			\\addplot [black,fill=gray] coordinates {(%s,%s) (%s,%s) (%s,%s) (%s,%s) (%s,%s)};
			\\legend{Accidentes de automóviles por color}
		\\end{axis}
	\\end{tikzpicture}
'
dibujo62 <- sprintf(dibujo62,colores[1],colores[2],colores[3],colores[4],colores[5],colores[1],accidentados[1], colores[2],accidentados[2],colores[3],accidentados[3],colores[4],accidentados[4],colores[5],accidentados[5])
} else {
questions<-solutions<-explanations<-NULL
questions[1]<-paste("de los automóviles color " ,segundo," será el segundo del más bajo al más alto.")
solutions[1]<-FALSE
...
include_tikz(dibujo62, name = "dibujo_62", markup = "markdown",format = typ, library = c("3d", "babel"), packages=c("pgfplots","tikz","xcolor"),width = "8cm")
```
