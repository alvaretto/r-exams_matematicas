...
========
\
En un curso de `r extudiamtes` estudiantes todos quieren sentarse en los `r eleggidos` asientos de la primera fila.
...
\
```{r image01csr01, warning=FALSE,echo = FALSE, results = "asis"}
include_tikz(image01csr01, name = "image01csr01", markup = "markdown",
             format = typ,library=c("arrows","perspective","datavisualization",                                    "calc","3d"),packages=c("tikz","booktabs","pgfplots",                                                                "tkz-euclide"), width = "11cm")
```
\
...
include_tikz(image02csr01, name = "image02csr01", markup = "markdown",format = typ, library = c("3d", "babel", "positioning"), packages=c("tikz","xcolor"),width = "11.5cm")
```
